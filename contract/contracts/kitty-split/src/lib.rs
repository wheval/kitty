...
#[contractimpl]
impl KittySplit {
    pub fn initialize(env: Env, native_token: Address, reputation_contract: Address) -> Result<(), Error> {
    ...
    pub fn pay_share(env: Env, split_id: u64, payer: Address) -> Result<(), Error> {
        ...
        env.events().publish((PAID_EVENT, split_id), (payer.clone(), amount));

        // Inter-contract call: report this payment to KittyReputation so the
        // payer's on-time-payment score is updated on-chain.
        let reputation_id: Address = env
            .storage()
            .instance()
            .get(&DataKey::ReputationContract)
            .ok_or(Error::NotInitialized)?;
        let reputation_client = reputation::Client::new(&env, &reputation_id);
        reputation_client.record_payment(&env.current_contract_address(), &payer, &amount);
